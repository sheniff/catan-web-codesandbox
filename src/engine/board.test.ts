...
describe("board", () => {
  ...
  it("should init all catan tiles and offset tiles", () => {
    ...
    expect(tiles["0,0,0"].tileType).toEqual(TileType.TILE);
    expect(tiles["-1,0,1"].tileType).toEqual(TileType.OFFSET);
    expect(tiles["0,-1,1"].tileType).toEqual(TileType.OFFSET);
    expect(tiles["-1,0,1"].tileType).toEqual(TileType.OFFSET);
    expect(tiles["0,1,-1"].tileType).toEqual(TileType.OFFSET);
    expect(tiles["1,-1,0"].tileType).toEqual(TileType.OFFSET);
    expect(tiles["1,0,-1"].tileType).toEqual(TileType.OFFSET);
  });
});
